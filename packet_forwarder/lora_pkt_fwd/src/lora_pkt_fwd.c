...
static int parse_gateway_configuration(const char * string, bool file);
...
static int parse_gateway_configuration(const char * string, bool file) {
    ...
    /* try to parse JSON */
    if (file) {
        root_val = json_parse_file_with_comments(string);
    } else {
        root_val = json_parse_string_with_comments(string);
    }
    if (root_val == NULL) {
        MSG("ERROR: %s is not a valid JSON file\n", (file) ? string : "string");
        exit(EXIT_FAILURE);
    ...
    conf_obj = json_object_get_object(json_value_get_object(root_val), conf_obj_name);
    if (conf_obj == NULL) {
        MSG("INFO: %s does not contain a JSON object named %s\n", (file) ? string : "string", conf_obj_name);
        return -1;
    } else {
...
const char *argp_program_bug_address = "<support@option.com>";
static char doc[] = "lora packet forwarder";

static struct argp_option options[] = {
    {"global", 'g', "FILE", 0, "global json file location", 0},
    ...
    {"offset", 'f', "NUM",  0, "frequency offset",          0},
    { 0 }
};

typedef struct arguments {
    char *global_cfg_path;  /* contain global (typ. network-wide) configuration */
    char *local_cfg_path;   /* contain node specific configuration, overwrite global parameters for parameters that are defined in both */
    char *debug_cfg_path;   /* if present, all other configuration files are ignored */
    char *json_string;      /* json string to parse*/
    bool public;            /* public/private network */
} arguments_t;
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
    ...
    switch (key) {
        case 'g':
            free(arguments->global_cfg_path);
            arguments->global_cfg_path = arg;
            break;
        case 'l':
            free(arguments->local_cfg_path);
            arguments->local_cfg_path = arg;
            break;
        case 'd':
            free(arguments->debug_cfg_path);
            arguments->debug_cfg_path = arg;
            break;
        case 'j':
            arguments->json_string = arg;
            break;
        case 'p':
            arguments->public = atoi(arg) > 0 ? true : false;
            break;
        case 'f':
            frequency_offset = atoi(arg);
            break;
        default:
...
int main(int argc, char *argv[])
{
    ...
    arguments_t args;
    {
        ...
        args.global_cfg_path = (char*) malloc(sizeof(global));
        strcpy(args.global_cfg_path, global);
        char local[] = "local_conf.json";
        args.local_cfg_path = (char*) malloc(sizeof(local));
        strcpy(args.local_cfg_path, local);
        char debug[] = "debug_conf.json";
        args.debug_cfg_path = (char*) malloc(sizeof(debug));
        strcpy(args.debug_cfg_path, debug);
        args.json_string = NULL;
        args.public = true;
    }
    argp_parse(&argp, argc, argv, 0, 0, &args);

    /* configuration file related */
    char *global_cfg_path = args.global_cfg_path; /* contain global (typ. network-wide) configuration */
    char *local_cfg_path  = args.local_cfg_path;  /* contain node specific configuration, overwrite global parameters for parameters that are defined in both */
    char *debug_cfg_path  = args.debug_cfg_path;  /* if present, all other configuration files are ignored */
    ...
    */
    ...
    if (access(debug_cfg_path, R_OK) == 0) { /* if there is a debug conf, parse only the debug conf */
        ...
        parse_gateway_configuration(debug_cfg_path, true);
        ...
        parse_gateway_configuration(global_cfg_path, true);
        if (access(local_cfg_path, R_OK) == 0) {
            ...
            parse_gateway_configuration(local_cfg_path, true);
        }
        if (args.json_string != NULL) {
            MSG("INFO: found json string, parsing it\n");
            MSG("INFO: redefined parameters will overwrite global and local parameters\n");
            parse_gateway_configuration(args.json_string, false);
        }
    } else if (access(local_cfg_path, R_OK) == 0) { /* if there is only a local conf, parse it and that's all */
        MSG("INFO: found local configuration file %s, parsing it\n", local_cfg_path);
        parse_SX1301_configuration(local_cfg_path);
        parse_gateway_configuration(local_cfg_path, true);
    } else if (args.json_string != NULL) {
        MSG("INFO: found json string, parsing it\n");
        parse_gateway_configuration(args.json_string, false);
    } else {
    ...
    // set public/private network, default is public
    // should be done after all configs are parsed and before the lgw is started
    if (lgw_board_setpublic(args.public) == LGW_HAL_SUCCESS) {
    ...
    if (argc > 1) {
        char *arg_end;
        arg_end = (char *) (argv[argc-1] + strlen(argv[argc-1]));
        *arg_end = ' ';
    }
    ...
    */
    ...
    if (stat_interval > 0) {
        ...
        while (!exit_sig && !quit_sig) {
            ...
            static uint32_t prev_trig_tstamp = 0;
            if ((prev_trig_tstamp!=0) && (trig_tstamp == prev_trig_tstamp)) {
                MSG("ERROR: PPS time stuck, quit forwarder: %u\n", trig_tstamp);
