...
void open_log(void) {
    ...
    sprintf(log_file_name, "%s/pktlog_%s_%s.csv", log_prefix, lgwm_str, iso_date);
...
void usage(void) {
    ...
    printf( " -f <string>  dirname of log file e.g. /tmp\n");
}
...
int main(int argc, char **argv)
{
    ...
    while ((i = getopt (argc, argv, "hr:f:")) != -1) {
        switch (i) {
            case 'h':
                usage();
                return EXIT_FAILURE;
